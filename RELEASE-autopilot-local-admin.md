...
| Skipped provisioning (standard user + Entra local-admin setting) for **classic** profiles | **Not documented.** Not on the Autopilot known-issues page either. Keep marked untested. | autopilot/known-issues |
...
2. **A documented local-admin conflict the article can cite outright.** From the Autopilot known-issues page: user-driven **hybrid** Entra join does not grant the user administrator rights even when the Autopilot profile specifies it, if another account on the device already has them (for example one created by a provisioning script or policy). That is Microsoft's own statement of "which setting wins" in one real case, and it is currently unsourced in the draft.
